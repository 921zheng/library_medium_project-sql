...
create proc dbo.bookCopiesAtAllSharpstowntwo (@booktitle varchar(70)='The Lost Tribe',@branchName varchar(70)='Sharpstown')
AS
select tbl_book.book_bookID as bookID, tbl_book.book_title as book_title, tbl_library_branch.library_branch_BranchName as branch_name, tbl_book_copies.book_copies_No_Of_Copies as numberof_copies
from tbl_book_copies
inner join tbl_book on tbl_book.book_BookID=tbl_book_copies.book_copies_BookID
inner join tbl_library_branch on tbl_library_branch.library_branch_BranchID=tbl_book_copies.book_copies_BranchID
where tbl_book.book_Title=@booktitle and tbl_library_branch.library_branch_BranchName=@branchName
GO
exec dbo.bookCopiesAtAllSharpstowntwo 

/* #2- How many copies of the book titled "The Lost Tribe" are owned by each library branch? */
create proc dbo.thelosttribeownedbranchthree (@bookname varchar(70)="The Lost Tribe")
AS
select tbl_book.book_Title,tbl_library_branch.library_branch_BranchName, tbl_book_copies.book_copies_No_Of_Copies
from tbl_book_copies
inner join tbl_book on tbl_book.book_BookID=tbl_book_copies.book_copies_BookID
inner join tbl_library_branch on tbl_library_branch.library_branch_BranchID=tbl_book_copies.book_copies_BranchID
where tbl_book.book_Title=@bookname

go 
exec dbo.thelosttribeownedbranchthree
...
create proc dbo.sharptownduedateistoday (@branchname varchar(70)="Sharpstown", @duedate varchar(50)="2/2/18")
AS
select tbl_book.book_Title,tbl_borrower.borrower_BorrowerName,tbl_borrower.borrower_BorrowerAddress
from tbl_book_loans
...
where tbl_library_branch.library_branch_BranchName=@branchname and tbl_book_loans.book_loans_DueDate=@duedate
GO
exec dbo.sharptownduedateistoday

/* #5- For each library branch, retrieve the branch name and the total number of books loaned out from that branch.  */
select tbl_library_branch.library_branch_BranchName,count(tbl_book_loans.book_loans_BookID) as total_number_books
...
create proc dbo.stephen (@authorname varchar(70)="Stephen King", @librarybranch varchar(70)="Central")
as 
select tbl_book.book_Title,tbl_library_branch.library_branch_BranchName,tbl_book_copies.book_copies_No_Of_Copies,tbl_book_authors.book_authors_AuthorName
from tbl_book_authors
inner join tbl_book on tbl_book.book_BookID=tbl_book_authors.book_authors_BookID
inner join tbl_book_copies on tbl_book_copies.book_copies_BookID=tbl_book_authors.book_authors_BookID
inner join tbl_library_branch on tbl_library_branch.library_branch_BranchID=tbl_book_copies.book_copies_BranchID
where tbl_book_authors.book_authors_AuthorName=@authorname and tbl_library_branch.library_branch_BranchName=@librarybranch
go
exec dbo.stephen